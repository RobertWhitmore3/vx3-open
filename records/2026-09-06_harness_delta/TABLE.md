...
The rubrics were ratified by the operator this afternoon ("i signed the rubrics.. run the bare and grounded table"); this is the run he ordered. Method: `probes/harness_delta.py` over the private pool (the operator's; never in this repository), every probe run bare and then grounded (a system message carrying `OATH.md` and `UNCERTAINTY.md`, digest `0f3ff54e679b1a08`, the model asked to read before answering). Scored here: the action channel only. Words are recorded verbatim in the run files; two reviewer families have scored them: `SCORES.md`, briefs and replies verbatim under `reviews/`. The "own king, proven voice" probe was skipped (placeholder in the pool). Nobody passes.
